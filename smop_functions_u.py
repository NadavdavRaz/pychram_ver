...
ALPHA = 0 # PI/2 #0.3 #amplitude of modulating signal
THETA =0 # PI/4 #phase offset of modulating signal
BETA = 0 # PI/5 #constant carrier phase offset
receiverKnowsCarrier= True; #If receiver knows the carrier frequency & phase offset
...
bits = np.array([1,0,1,1,0,0,0,1,0,1,0,1,0,1]*10)
to_send = multiply(bits)

final_sample_list = modulation(to_send, "FM")
play_data(final_sample_list)

save_wave(SAMPLE_RATE, final_sample_list, 'FM_test.wav')


# In[616]:


record("rec/FM-test_rec.wav",5)


# In[617]:


frame_rate, data = wavfile.read("rec/FM-test_rec.wav")
data = data[:,1]
# data = data[20000:]
n_frames = len(final_sample_list)
eps = find_thresh(data, n_frames, 42)

print(eps)
data = cut_irellevant(data, thresh = eps, draw = True)
...
# frame_rate, data = wavfile.read("record_30bits_3times.wav")
# data = data[:,1]
# data = data[20000:]
# eps = find_thresh(data, 1260)
# print(eps)
# data = cut_irellevant(data, thresh = eps, draw = True)
# print(len(final_sample_list))
# print(len(data))
...
# dat = redecyherPM(DecypherPM(data, thresh = 1))
# dat = decide(dat)
# err = np.array(dat)- bits
# n_of_err = np.count_nonzero(err != 0)
...
def DecypherFreqShift(data, ChunkSize = CHUNK_SIZE, thresh = 4):
...
dat = RestoreChunks(DecypherFreqShift(data), int(BASIC_TIME*SAMPLE_RATE/CHUNK_SIZE))
dat = decide(dat)
dat = [1] + dat
print(dat)
...
frame_rate, data = wavfile.read("rec/test_am1_140_bit_recorded.wav")
data = data[:, 0]
